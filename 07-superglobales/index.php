...
<body>
    ...
    <div class="container">
        ...
        <a class="btn btn-primary" href="index.php?name=titi">Hello Titi</a>
        <a class="btn btn-primary" href="index.php">Hello World</a>

        <?php 
            // $_GET permet de récupèrer tous les paramètre de l'URL. Les paramètres se définissent en accèdant à l'URL de cette manière :
            // index.php?name=toto&id=10
    ...
    <?php 
        ...
        $search = $_GET['search'] ?? ''; // Opérateur null coalesce (equivalent au ternaire)
    ?>
    ...
    <form method="post" action="">
        ...
        <div class="form-check">
                <input type="checkbox" name="skills[]" class="form-check-input" id="front" <?= in_array('front', $skills) ? 'checked' : ''; ?>> 
                <label for="front" class="form-check-label">Front</label> 
        </div>
        <div class="form-check">
                <input type="checkbox" name="skills[]" class="form-check-input" id="back"> 
    ...
    if (!empty($_POST)) {//On vérifie si le formulaire est bien soumis
            ...
            // On va créer un tableau d'erreurs vide qu'on va pouvoir remplir au fur et à mesure des verifications
            $errors = [];
